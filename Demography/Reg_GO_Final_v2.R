...
# Predictors: Intercept, effective population size, migration, and diversity.
# No interactions.
...
working_directory<-"/Users/kenengoodwin/Desktop/USU/Graduate/Projects/Butterflies/Analyses/Models/GO_Correlations/GO_Final"
...
# Supply population identifiers as row names.
row.names(covariates)<-covariates$PopID

# Subset to relevant fields.
covariates<-covariates[,c(response,"Ne","Migration","Diversity")]
...
scaling<-data.frame(Predictor=c("Ne","Migration","Diversity"),
                    Mean=c(Ne_mean,migration_mean,diversity_mean),
                    SD=c(Ne_SD,migration_SD,diversity_SD))
...
# Create the predictor matrix without interactions between the predictor variables.
X<-model.matrix(~Ne+Migration+Diversity,data=covariates)
...
mod.fit$weights<-exp(-0.5*mod.fit$delta.neg2xloo_loglik)/sum(exp(-0.5*mod.fit$delta.neg2xloo_loglik))

# Compute predictor correlations.
rho<-as.data.frame(cor(X[,colnames(X)!="Intercept"]))
...
write.csv(x=rho,file="Output/rho.csv",row.names=TRUE)
